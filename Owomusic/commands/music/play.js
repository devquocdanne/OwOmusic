	...
	run: async ({ interaction, client }) => {
        ...
        if (player && player.voiceChannel !== interaction.member.voice.channelId) {
            return interaction.editReply({ 
				embeds: [embed.setDescription("\`❌\` | Bạn phải ở cùng kênh thoại với bot.")], 
				ephemeral: true
			});
        } else if (!player) {
		...
		if (loadType === "playlist") {
			...
			return interaction.editReply({ embeds: [embed.setDescription("\`❌\` | Không tìm thấy kết quả nào cho truy vấn của bạn.")] });
		}
  	},
	options: {
		inVoice: true,

...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("queue")
        .setDescription("Xem hàng đợi và bài hát đang phát")
        .setDMPermission(false),

    run: async ({ interaction, client }) => {
        try {
            await interaction.deferReply();
            await interaction.editReply({ content: "`🔎` | Đang tải hàng đợi..." });

            const player = client.riffy.players.get(interaction.guildId);

            if (!player || !player.queue || !player.queue.length) {
                return interaction.editReply({ 
                    content: "`❌` | Hiện tại không có bài hát nào đang phát.",  
                    ephemeral: true 
                });
            }
                ...
                const songsWithThumbnails = await Promise.all(
                    songsToDraw.map(async (track) => {
                        ...
                        try {
                            ...
                            logger("Không tải được hình thu nhỏ", "warn");
                        }
                        return {
                            ...track,
                            thumbnailImage
            ...
            const filter = async (buttonInteraction) => {
                if (buttonInteraction.user.id !== interaction.user.id) {
                    await buttonInteraction.reply({ content: "`❌` | Bạn không thể sử dụng các điều khiển này vì bạn không khởi tạo lệnh. Sử dụng /queue để xem hàng đợi.", ephemeral: true });
                    return false;
                }
                return true;
            };
